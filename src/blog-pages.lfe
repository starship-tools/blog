...
(defun get-page
  ...
  (('archives)
    (clj:-> (blog-data:base "Archives")
            (lists:append `(#(active "archives")))
            (blank-tmpl:render)
            (get-content)))
  (('categories)
    (clj:-> (blog-data:base "Categories")
            (lists:append `(#(active "categories")))
            (blank-tmpl:render)
            (get-content)))
  (('tags)
    (clj:-> (blog-data:base "Tags")
            (lists:append `(#(active "tags")))
            (blank-tmpl:render)
            (get-content)))
  (('authors)
    (clj:-> (blog-data:base "Authors")
            (lists:append `(#(active "authors")))
            (blank-tmpl:render)
            (get-content)))
  (('about)
    (clj:-> (blog-data:base "About")
            (lists:append `(#(active "about")))
            (blank-tmpl:render)
            (get-content)))
  (('timeline)
